...
<nav
	class="fixed top-0 right-0 left-0 z-50 flex items-center justify-between border-b border-white/6 bg-[rgba(24,25,38,0.55)] px-[clamp(20px,5vw,64px)] py-[22px] backdrop-blur-[10px]"
>
	<a
		href="/"
...
{#if menuOpen}
	<div
		class="fixed inset-0 z-49 flex flex-col items-start justify-center gap-7 bg-[rgba(24,25,38,0.98)] px-[clamp(24px,8vw,64px)] py-8 backdrop-blur-[10px] min-[721px]:hidden"
	>
		{#each navItems as item (item.href)}
			<a
				href={item.href}
